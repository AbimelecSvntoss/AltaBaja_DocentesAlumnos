Public Class Archivos
    ...
    '' Metodo para almacenar la información de los alumnos en un archivo de texto
    ...
    '' Metodo para importar la información de los alumnos desde un archivo de texto
    Public Shared Function ImportarAlumnos(rutaArchivo As String) As List(Of Alumno)
        ...
        If System.IO.File.Exists(rutaArchivo) Then
            Using reader As New System.IO.StreamReader(rutaArchivo)
                While Not reader.EndOfStream
                    Dim linea As String = reader.ReadLine()
                    ' Espera formato: "ID: 1, Nombre: Juan, Apellidos: Pérez, Teléfono: 1234567890, Especialidad: Ingeniería, Matrícula: 2021001"
                    Dim partes As String() = linea.Split(New String() {", "}, StringSplitOptions.None)
                    If partes.Length >= 6 Then
                        Dim id As Integer = Convert.ToInt32(partes(0).Replace("ID: ", ""))
                        Dim nombre As String = partes(1).Replace("Nombre: ", "")
